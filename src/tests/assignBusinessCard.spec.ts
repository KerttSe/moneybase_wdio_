 ...
 * default existing business member. Each it() is a checkpoint in ONE
 * continuous session (login + ensureBusinessAccount run once in `before`,
 * not per test). No manual Select User step and no Spend From step for
 * Physical cards.
...
describe('Business Cards - Assign Physical card', function () {
  ...
  it('ABC-1.5 Continue with default assignee', async function () {
    await BusinessCardPage.continueWithDefaultAssignee()
  })

  it('ABC-1.6 Order default card design', async function () {
    await BusinessCardPage.orderDefaultCardDesign()
  })

  it('ABC-1.7 Confirm assignee address and submit', async function () {
    await BusinessCardPage.confirmAssigneeAddressAndSubmit()
  })

  it('ABC-1.8 Verify success modal and close', async function () {
    await BusinessCardPage.verifySuccessAndClose()
  })

  it('ABC-1.9 Wait for created card details screen', async function () {
    await BusinessCardPage.waitForCreatedCardReady()
  })

  it('ABC-1.10 Freeze created card', async function () {
    await BusinessCardPage.tapFreeze()
  })

  it('ABC-1.11 Delete created card via report/block', async function () {
    await BusinessCardPage.tapReport()
    await BusinessCardPage.verifyCardBlocked()
  })

  it('ABC-1.12 Return to cards list', async function () {
    await BusinessCardPage.tapViewMyCards()
  })

  it('ABC-1.13 Verify created card was removed', async function () {
